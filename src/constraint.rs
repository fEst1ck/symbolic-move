...
use crate::{dynamic::BranchCondition, value::Value};
...
impl<'ctx, T> Constrained<'ctx, T> {
    ...
    where
        F: Fn(T) -> Constraint<'ctx>,
    {
        self.map(pred).to_constraint()
    }
}

pub fn map2_constrained<'ctx, A, B, T, F>(
    f: F,
    arg: (Constrained<'ctx, A>, Constrained<'ctx, B>),
) -> Constrained<'ctx, T>
where
    F: Fn((A, B)) -> T,
{
...
impl<'ctx> Disjoints<'ctx, Constraint<'ctx>> {
    ...
    pub fn to_constraint(&self, ctx: &'ctx Context) -> Constraint<'ctx> {
        self.iter()
            .filter_map(|x| x.to_constraint_opt().map(|x| x.simplify()))
            .fold(Bool::from_bool(ctx, false), |acc, x| acc | x)
    }
...
pub fn fmap2_disjoints<'ctx, A: Clone, B: Clone, C, F>(
    f: F,
    args: (&Disjoints<'ctx, A>, &Disjoints<'ctx, B>),
) -> Disjoints<'ctx, C>
where
    F: Fn((A, B)) -> C + Clone,
{
