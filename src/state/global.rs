use crate::traits::{Functor, Applicative, Monoidal};
...
pub struct GlobalMemory<'ctx> {
    ...
    pub resource_value: BTreeMap<ResourceId, SymbolicTree<OrderedConstraint<'ctx>, Array<'ctx>>>,
    /// Map from account address to bool, where false indicates the account doesn't hold the resource
    pub resource_existence: BTreeMap<ResourceId, SymbolicTree<OrderedConstraint<'ctx>, Array<'ctx>>>,
}
...
impl<'ctx> GlobalMemory<'ctx> {
    ...
    fn init_resource_value<'env>(&mut self, datatypes: Rc<RefCell<Datatypes<'ctx, 'env>>>, resource: &ResourceId) {
        let init_val = Array::fresh_const(
            self.get_ctx(),
            "global memory",
    ...
    pub fn get_resource_existence(
        &mut self,
        resource: &ResourceId,
    ) -> SymbolicTree<OrderedConstraint<'ctx>, Array<'ctx>> {
    ...
    pub fn exists(
        ...
        addr: SymbolicTree<OrderedConstraint<'ctx>, Value<'ctx>>,
    ) -> SymbolicTree<OrderedConstraint<'ctx>, Constraint<'ctx>> {
        let resource_existence = self.get_resource_existence(resource_type);
        resource_existence.lift_a2(addr, |array, value| {
            array
    ...
    pub fn real_move_to<'env>(
        &mut self,
        resource_type: &ResourceId,
        addrs: SymbolicTree<OrderedConstraint<'ctx>, BV<'ctx>>,
        resource: SymbolicTree<OrderedConstraint<'ctx>, Datatype<'ctx>>,
        datatypes: Rc<RefCell<Datatypes<'ctx, 'env>>>
    ) {
