            ...
            where F: Fn(Self::Source) -> B
    {
        match self {
            ...
            SymbolicTree::Leaf(l, p, r) => {
                SymbolicTree::Leaf(Box::new(l.map(f)), p, Box::new(r.map(f)))
            }
        }
    }
}
    ...
    fn prod<B: Clone>(self, other: Self::Fb<B>) -> Self::Fb<(Self::Source, B)> {
        ...
        match (self, other) {
            ...
            (t1@Node(_), Leaf(l, p, r)) => {
                Leaf(Box::new(t1.prod(*l)), p, Box::new(t1.prod(*r)))
            }
            (Leaf(l, p, r), t2@Node(_)) => {
                Leaf(Box::new(l.prod(t2)), p, Box::new(r.prod(t2)))
            }
            (t1@Leaf(l1, p1, r1), t2@Leaf(l2, p2, r2)) => {
                if p1 < p2 {
                    Leaf(Box::new(t1.clone().prod(*l2)), p2, Box::new(t1.prod(*r2)))
                } else if p1 == p2 {
                    Leaf(Box::new(l1.prod(*l2)), p1, Box::new(r1.prod(*r2)))
                } else {
